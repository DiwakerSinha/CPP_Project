...
void Allocator::loadRooms(const std::string& file)
{
    ...
    while (fileio::areMoreRooms()) {
        //i++; we cannot increment because we would not loaded in the information for the first room at rooms[0]
        rooms[i] = fileio::nextRoom();
        totalCapacity += rooms[i].capacity;
        i++; //added the line here because we can now safely increment 
    }
}
...
//Now we need to deallocate and remove the memory to avoid a leak.

Allocator::~Allocator() //We are deleting all the memory inside an Allocator Object.
{
  delete []rooms; // deallocating memory for the room array so we avoid a leak
  delete []alpha; // deallocating memory for the alphabets array so we avoid a leak
  rooms = NULL; // making sure pointer is not pointing to random data
  alpha = NULL; // making sure pointer is not pointing to random data
}